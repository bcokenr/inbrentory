    ...
    <main className={[styles.sometypeMono, "space-y-6"].join(" ")}>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
        <div className="rounded-lg bg-white p-4 shadow-sm">
          ...
          <div className="font-medium">{format(new Date(tx.createdAt), 'MM/dd/yyyy')}</div>
          ...
          <div className="font-medium">${Number(tx.subtotal ?? 0).toFixed(2)}</div>

          <div className="mt-3 text-sm text-gray-500">Store credit applied</div>
          <div className="font-medium">${Number(tx.storeCreditAmountApplied ?? 0).toFixed(2)}</div>

          <div className="mt-3 text-sm text-gray-500">Total</div>
          <div className="font-medium">${Number(tx.total ?? 0).toFixed(2)}</div>
        </div>
